...
# 删除一列
# inplace 默认是false  返回新的表, 如果表很大 就不返回新的表,用原来的表修改
# print(df.drop(columns='new_col', inplace=True))
...
# print(table)
# 根据哪一个字段进行分组
# print(table.groupby(by='Name'))  # 返回groupby对象
# 分组的目的就是聚合(sin,sum,avg)
# print(table.groupby(by='Name').sum())
# print(table.groupby(by='Name').std())
# print(table.groupby(by='Name')['Python'].std())
# print(table.groupby(by='Name')['Python'].mean())

# 如果不想把name变成索引,使用as_index
# print(table.groupby(by='Name', as_index=False)['Python'].mean())
# 对多列进行分组
# print(table.groupby(by=['Name', 'Python'], as_index=False)['Python'].mean())

########################################################
# 排序，降序时使用ascending(英文表示上升)
# print(table.sort_values(by='Linux'))
# print(table.sort_values(by='Linux', ascending=False))
print(table.sort_values(by=['Name', 'Linux'], ascending=False))
